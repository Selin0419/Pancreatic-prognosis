...
# ------------------------------
manual_kfold <- function(data, formula, k = 10, cost = C, gamma = gamma_value) {
  set.seed(42)
  n <- nrow(data)
  ...
  for(i in 1:k) {
    ...
    model <- ksvm(formula, data = train_data, kernel = "rbfdot", 
                  kpar = list(sigma = gamma_value), C = C)
...
#kfold CV using RBF
kfold_res <- manual_kfold(df, svm_formula, k = 10, cost = C, gamma = gamma_value)
...
manual_loocv_rbf <- function(data, formula, cost = C, gamma = gamma_value) {
  n <- nrow(data)
  preds <- rep(NA, n)
  for(i in 1:n) {
    train_i <- data[-i, ]
    ...
    model <- ksvm(formula, data = train_i, kernel = "rbfdot", 
                           kpar = list(sigma = gamma_value), C = C)
...
loocv_res <- manual_loocv_rbf(df, svm_formula, cost = C, gamma = gamma_value)
...
cat("Log-rank test (LOOCV):", logrank_p_loocv,"\n")
...
ggsave("SVM_rbf_loocv.png", plot = km_plot$plot, width = 7, height = 6, units = "in", dpi = 300)
