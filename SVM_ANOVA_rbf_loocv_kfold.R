...
manual_kfold <- function(data, formula, k = 10, C = 2, gamma = gamma_value) {
  ...
  for(i in 1:k) {
    ...
    model <- ksvm(formula, data = train_data, kernel = "anovadot", 
                  kpar = list(sigma = gamma_value, degree = 1), C = C)
...
manual_loocv_rbf <- function(data, formula, cost = 2, gamma = gamma_value) {
  ...
  for(i in 1:n) {
    ...
    model <- ksvm(formula, data = train_i, kernel = "anovadot", 
                  kpar = list(sigma = gamma_value, degree = 1), C = C)
...
ggsave("SVM_ANOVA_rbf_loocv.png", plot = km_plot$plot, width = 7, height = 6, units = "in", dpi = 300)
