...
manual_kfold <- function(data, formula, k = 10, C = 2, gamma = gamma_value) {
  ...
  for(i in 1:k) {
    ...
    model <- ksvm(formula, data = train_data, kernel = "tanhdot", 
                  kpar = list(scale = 1, offset = 0), C = C, gamma = gamma_value)
...
manual_loocv_rbf <- function(data, formula, cost = 2, gamma = gamma_value) {
  ...
  for(i in 1:n) {
    ...
    model <- ksvm(formula, data = train_i, kernel = "tanhdot", 
                  kpar = list(scale = 1, offset = 0), C = C, gamma = gamma_value)
...
ggsave("SVM_Hyperbolic_tangent_kernel_loocv.png", plot = km_plot$plot, width = 7, height = 6, units = "in", dpi = 300)
